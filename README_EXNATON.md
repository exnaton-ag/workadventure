...
| **Accessories, hats & clothes** | `play/src/pusher/data/woka.json` (adds `hat_santa_red`, `hat_brain`, `accessory_trophy`, `accessory_blue_lightsaber`, `clothes_beige_tunic`) + PNGs in `play/public/resources/customisation/character_hats/`, `.../character_accessories/` and `.../character_clothes/` |
...
4. **App smoke test** (optional) — run the stack (Colima needs ≥8 GiB RAM) and confirm the new companions + santa/brain hats + trophy/lightsaber accessories + beige tunic clothes item appear in the WOKA customization screen.
